...
<template>
  <div>
    ...
    <div class="grid md:grid-cols-2 gap-6 mb-8">
      <Card>
        <CardContent class="p-5">
          ...
          <h2 class="text-2xl font-bold">{{ stats.total_events }}</h2>
        </CardContent>
      </Card>
    ...
    <!-- EVENT -->
    <div>
      <h2 class="text-xl font-semibold mb-4">Semua Event</h2>
      ...
      <div v-else class="grid md:grid-cols-3 gap-6">
        <Card v-for="event in events" :key="event.id">
          <CardContent class="p-5">
            ...
            <p class="text-sm text-gray-500">📍 {{ getLocation(event) }}</p>

            <Link :href="`/events/${event.id}`">
              <Button class="mt-3 w-full">Detail</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
